...
class Rail {
  constructor(splineDescription, bead, camera, renderer) {
    this.camera = camera
  ...
  scene.traverse(obj => {
    if (obj.name.startsWith("Bead")) {
      ...
      if (obj.name.includes("Blue")) {
        const rail = new Rail(RAIL_RED, obj, camera, renderer)
        state.rails.push(rail)
      }

      else if (obj.name.includes("Red")) {
        const rail = new Rail(RAIL_YELLOW, obj, camera, renderer)
        state.rails.push(rail)
      }

      else if (obj.name.includes("Yellow")) {
        const rail = new Rail(RAIL_BLUE, obj, camera, renderer)
        state.rails.push(rail)
      }
    }
  })
