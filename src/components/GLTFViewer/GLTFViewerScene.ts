...
export class GLTFViewerScene {
    ...
    constructor(options: GLTFViewerSceneOptions) {
        ...
        if (this.options.clearColor) this.renderer.setClearColor(new THREE.Color(this.options.clearColor))
    ...
    init(container: HTMLElement) {
        ...
        this.scene.traverse(o => {
            ...
            else if (o instanceof THREE.Mesh && o.material instanceof THREE.MeshPhysicalMaterial) {
        ...
        const cameraTarget = this.gltf.scene.children.find(o => o.userData.name === 'CameraTarget')
        if (cameraTarget) {
        ...
        if (this.options.zoomable) {
            this.controls.minDistance = distance * .5
    ...
    async setEnvironment(environment: THREE.DataTexture | string) {
        this.environmentTexture = typeof environment === 'string'
        ...
        const cubeRenderTarget = pmremGenerator.fromEquirectangular(this.environmentTexture)
        const envMap = cubeRenderTarget.texture ?? null

        this.scene.environment = envMap
        if (this.options.environmentIsBackground) this.scene.background = this.environmentTexture
