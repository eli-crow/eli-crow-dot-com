...
<template>
    <Card>
        ...
        <div class="p-8 pt-0">
            <h2 class="text-xl leading-snug font-light text-gray-900 mb-2">{{ props.title }}</h2>
            <p v-if="props.byline">
                <time>{{ props.byline }}</time>
            </p>
        </div>
        ...
        <Modal :title="props.title" :is-open="state.modalIsOpen" @close="state.modalIsOpen = false">
            <GLTFViewer :gltf="props.gltf" auto-rotate />
        </Modal>
    </Card>
</template>