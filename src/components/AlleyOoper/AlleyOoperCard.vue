    ...
    <div class="card-padding">
      <div class="alley-ooper-group">
        <label class="alley-ooper">
          ...
          <AlleyOoper class="alley-ooper-input" v-model="alleyOopers[0]" min="0" max="100" :spline="[[0,0, .33,0, .66,0, 1,0]]" style="height: 1rem;" />
        </label>
        <label class="alley-ooper">
          <span class="alley-ooper-label">Swooper</span>
          <span class="alley-ooper-value">{{ alleyOopers[1].toFixed(0) }}</span>
          <AlleyOoper class="alley-ooper-input" v-model="alleyOopers[1]" min="0" max="100" :spline="[[0,0, .33,1, .66,1, 1,0]]" style="height: 1.5rem;" />
        </label>
        <label class="alley-ooper">
          <span class="alley-ooper-label">Swisher</span>
          <span class="alley-ooper-value">{{ alleyOopers[2].toFixed(0) }}</span>
          <AlleyOoper class="alley-ooper-input" v-model="alleyOopers[2]" min="0" max="100" :spline="[[0,1, 1.75,1.5, -0.2,-0.15, 1,0]]" style="height: 3rem;" />
        </label>
        <label class="alley-ooper">
          <span class="alley-ooper-label">Looper</span>
          <span class="alley-ooper-value">{{ alleyOopers[3].toFixed(0) }}</span>
          <AlleyOoper class="alley-ooper-input" v-model="alleyOopers[3]" min="0" max="100" :spline="[[0,1, 2,-0.85, -0.5,-1.33, 1,1]]" style="height: 3rem;" />
        </label>
      </div>
    </div>
